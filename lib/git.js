...
export async function branchExists(branch, execaOptions) {
  try {
    const { exitCode } = await execa("git", ["show-ref", "--verify", `refs/remotes/origin/${branch}`], execaOptions);
    return exitCode === 0;
  } catch (error) {
    debug(error);
    return false;
...
export async function createBranch(branch, baseBranch, execaOptions) {
  ...
  debug(`Creating branch ${branch} from ${baseBranch}`);
  // Create a fresh branch from the base branch
  await execa("git", ["checkout", "-b", branch, baseBranch], execaOptions);
  debug(`Created branch ${branch}`);
}

export async function forceUpdateBranch(branch, baseBranch, execaOptions) {
  await execa("git", ["checkout", branch], execaOptions);
  await execa("git", ["reset", "--hard", baseBranch], execaOptions);
}

export async function pushBranch(repositoryUrl, refspec, force = false, execaOptions) {
  const args = ["push"];
  if (force) args.push("--force");
  args.push(repositoryUrl, refspec);
  await execa("git", args, execaOptions);
}

export async function checkout(branch, execaOptions) {
  try {
    // Clean untracked files to prevent checkout conflicts
    await execa("git", ["clean", "-f"], execaOptions);
    await execa("git", ["checkout", branch], execaOptions);
  } catch (error) {
    debug(error);
    throw error;
 ...
 */
export async function push(repositoryUrl, execaOptions, options = {}) {
  if (options.skipGitPush) {
    debug('Skipping git push due to --skip-git-push flag');
    return;
  }
 ...
 * @param {Boolean} [options.skipGitPush] Whether to skip pushing to git.
 *
 * @throws {Error} if the push failed.
 */
export async function pushNotes(repositoryUrl, ref, execaOptions, options = {}) {
  if (options.skipGitPush) {
    debug('Skipping git push notes due to --skip-git-push flag');
    return;
  }
